...
class SerieTemporal:
    ...
    def serie_temporal(self, titulo=None, x_label=None, y_label=None, inpc='INPC'):
        ...
        periodo_inpc = self._inpc['data'].dt.year.between(self._periodo[0], self._periodo[1]) == True
        ...
        fig.add_trace(go.Scatter(x=self._investimento[periodo_investimento][self._x_investimento], 
                    ...
                    mode='lines',
                    name='Poupança'))
        fig.add_trace(go.Scatter(x=self._inpc[periodo_inpc][self._x_inpc], 
                                y=self._inpc[periodo_inpc][self._y_inpc],
                    mode='lines',
                    name=inpc,
        ...
        fig.add_trace(go.Indicator(
                    ...
                    delta = {"reference": np.sum(self._inpc[periodo_inpc][self._y_inpc]).round(2)},
                    title = {"text": "Rendimento relativo a inflação"},
                    domain = {'y': [0, 1], 'x': [0.25, 0.75]}))
        fig.update_layout(title=titulo,
                        xaxis_title=x_label,
                        yaxis_title=y_label)
        st.plotly_chart(fig, hidth=1000)
